...
{
    private new Vector3Int scatterCell = new Vector3Int(-15, -15, 0);
    private new int minPellets = 80;
    //private new int minPellets = 0;

    public Clyde(GameManager gameManager) : base(gameManager) { }
    ...
    public override Ghost.Mode ChooseMode(Ghost.Mode currentMode, int seconds, int pelletsEaten, bool intoPowerMode, Vector3Int currentCell, bool wasEaten, Vector3Int ghostHouseCell, int stage)
    {
        ...
        if (cronogramIndex > 4)
        {
            if (Vector3.Distance(gameManager.grid.CellToWorld(currentCell), gameManager.player.currentCell) > 8f)
